...
// Function to retrieve the min value from a packet, validate CRC, and update the main distance array
  ...
  while (millis() - start_time < parsing_time_limit && buffer_count >= PACKET_SIZE) {
    ...
    if (raw_array[read_index] == 0x54 && raw_array[(read_index + 1) % BUFFER_SIZE] == 0x2C) {
      ...
      if (buffer_count >= PACKET_SIZE) {
        ...
        // Calculate CRC8 checksum for the packet (excluding the last byte, which is the CRC)
        byte computed_crc = calculate_crc8(packet, PACKET_SIZE - 1);

        // Compare the computed CRC with the one in the packet
        if (computed_crc == packet[PACKET_SIZE - 1]) {
          // Process the packet if CRC is valid
          ...
          // Handle CRC failure (optional logging)
...
static const uint8_t CrcTable[256] =
{
    ...
    0x7b, 0x27, 0x6a, 0xbd, 0xf0, 0x5e, 0x13, 0xc4, 0x89, 0x63, 0x20, 0xf9,
...
uint8_t calculate_crc8(const uint8_t *data, uint8_t len) {
    uint8_t crc = 0; // Initialize CRC to 0 as per the manual
    for (uint8_t i = 0; i < len; i++) {
        crc = CrcTable[(crc ^ data[i]) & 0xFF]; // Lookup table-based CRC
    }
    return crc;
}
